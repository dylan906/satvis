...
# r1 = stack((-sin(t), cos(t), zeros(n)), axis=1) # complicated case
r1_dot = stack([zeros(n), zeros(n), zeros(n)], axis=1)
# r1_dot = stack((-cos(t), -sin(t), zeros(n)), axis=1) # complicated case
r1_mag_dot = zeros(n)
...
# Propagate motion and calculate visibility
...
# get basic visibility history (no derivative)
...
for i, (d, label) in enumerate(zip(data, labels)):
    ...
    axs[row, col].scatter(negative_indices, negative_values, color="blue", marker="s")
